...
const Department = {
  ...
  getDepartment: (id: string) => requests.get(`departments/${id}`),
...
const SuperAdmin = {
  ...
  createSystem:(info:any)=>requests.post("system",info)
};
const Company = {
...
const Team = {
  ...
  leaves: (info: any) => requests.post("user/teammates/leave", info),
  profile: () => requests.get("admin/company"),
  updateCompany: (id: string, info: any) =>
    requests.patch(`admin/company/${id}`, info),
  ...
  deleteArchive: (id: any,info:any) => requests.patch(`user/teammate/process/${id}`,info)
};
const Auth = {
...
const User = {
  ...
  allUserListing:(q?:any)=>requests.get(`user/all/users${q ? `?${q}` : ""}`)
};
...
const Process = {
  ...
  editProcess:(id:string,info:any)=>{
...
/${id}`),
  ...
  patchimportexport:(info:any)=>requests.patch(`process/import-export`,info),
