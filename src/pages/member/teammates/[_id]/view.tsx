...
const View = (props: any) => {
  ...
  function correctUserRole(str:string){
    return str.split('_').map((word:any)=>word.slice(0,1)+word.slice(1).toLowerCase()).join(' ')+" Details"
  }
      ...
      <section className="teammates_detail">
        <div className="container-fluid">
          <Row gutter={[20, 20]} justify={"space-between"} align={"middle"}>
            <Col span={24}>
              <TypographyTitle level={4}>{correctUserRole(teamMateDetail.type)}</TypographyTitle>
            </Col>
            ...
            <Col span={24} lg={10} xl={10} xxl={8}>
              <Space direction="vertical" className="w-100">
                ...
                {/* <Link
                  href={`/${userType}/privilege/page/1${
                    teamMateDetail?.name ? `?search=${teamMateDetail.name}` : ""
                  }`}
                >
                  <Button
                    type="primary"
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    console.log(
      cookies["COOKIES_RAIZE_ACCESS_TOKEN"],
