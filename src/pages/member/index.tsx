...
import { Avatar, Spin, Button, Card, Col, Divider, Flex, Input, Modal, Row, Space, TypographyText, TypographyTitle, Upload } from "@/lib/AntRegistry";
...
const Home = () => {
  ...
  const initData = async () => {
    ...
    try {
      ...
      console.log(apiRes, "propspropsprops")
    } catch (error) {
  ...
  const statsData = [
    ...
    {
      count: state?.shared_process ? state.shared_process : 0,
      statsName: 'Shared Process',
      href: `/${userType}/process/list/all/1`
    },
    // {
    //   count: '28 / 100',
    //   statsName: 'Total Processes'
    // },
    {
      count: state?.total_systems ? state.total_systems : 100,
      statsName: 'Department Systems',
      href: `/${userType}`
    },
  ]
                ...
                {Array.isArray(state?.alert) && state?.alert?.length ? (
                  state?.alert?.slice(0, 6)?.map((res: any, index: any) => (
                    <Col key={index} span={24}>
                      <AlertCard {...res} setOpenModal={setOpenModal} setRejectedId={setRejectedId} />
                    </Col>
                  ))
                ) : (
