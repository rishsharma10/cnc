...
const ProcessPage = () => {
  ...
  const deleteProcess = async (_id: string) => {
    setLoading(true);
    try {
      if(router.query.type == "archive"){
        const apiRes = await henceforthApi.Process.archieved(String(_id),{});
        Toast.success("Process Restore successfully")
      }else{
  ...
  const itemsRes = (res: any) => [
    {
      ...
      label: (
        <Button
          ...
          onClick={() => router.replace(`/${userType}/process/${res._id}/details`)}
        >
    ...
    },
    {
      ...
      label: (
        ...
        >
          ...
          <TypographyText className="ms-2 fw-semibold">{router.query.type == "deleted" ? "Restore" :router.query.type == "archive" ? "Restore" : ""}</TypographyText>
        </Button>
      ),
    }
    // {
    ...
    state?.data.map((res: any, i: number) => {
      return {
        key: router.query.pagination
          ...
          : i + 1,
        name: (
          <span style={{color:"#9778F7"}} role="button" title={res?.title} onClick={() => router.replace(`/${userType}/process/${res._id}/details`)}>
        ...
        process_owner: (
          <Flex align="center" gap={8}>
            <Tooltip title={res?.creator_id?.name} className="border-0">
            <Avatar
              src={
            ...
            <Row gutter={[20, 20]}>
              <Col span={24}>
                <Flex justify="space-between" align="center" gap={10}>
                  ...
                  {router.query.type==="deleted"?"Deleted":"Archived"} Processes 
                  </TypographyTitle>
