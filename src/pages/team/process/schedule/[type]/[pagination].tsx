    ...
    state?.data.map((res: any, i: number) => {
      return {
        key: router.query.pagination
          ...
          : i + 1,
        name: (
          <span style={{color:"#9778F7"}} role="button" title={res?.process_id?.title} onClick={() => router.replace(`/${userType}/process/${res?.process_id._id}/details`)}>
        ...
        no_of_ignore: (
          <span className="text-capitalize">
            {res?.ignore_type == "IGNORE_COMPLETELY" ? "NULL"  :res?.no_of_ignore ?? "--"}
          </span>
        ),
