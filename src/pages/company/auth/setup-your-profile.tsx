...
const SetupYourProfile = () => {
  ...
  const { loading, setLoading, userInfo, setUserInfo } = useContext(GlobalContext);
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      console.log("profilesetpdetails", apiRes);
      if (!apiRes?.data?.is_department_added) {

        router.replace(`/company/auth/department`);
      } else {
        router.replace(`/company/add-team-admin/${apiRes?.data?.department_id?._id}`)
      }
    } catch (error) {
      setLoading(false);
    }
