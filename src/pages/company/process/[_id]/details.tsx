...
const View = (props: any) => {
  ...
  const handleProcessTitleChange = async (value: any) => {
    try {
      let apiRes = await henceforthApi.Process.editProcess(
        ...
        {description: value.process_desc }
      ) as any;
  ...
  let timer :any;
  const debounceTitleChange=async(value:any)=>{
    if(timer){
      clearTimeout(timer)
    }

    timer=setTimeout(()=>{
       handleTitleChangeforDesc(value.target.value)
    },700)
  }
          ...
          <Row gutter={[10, 0]}>
            <Col span={24}>
              <FormItem name="process_title"
                ...
                <Input onChange={(e:any)=>debounceTitleChange(e)} className="mb-3 border" placeholder="Enter title" />
              </FormItem>
            </Col>
            <Col span={24}>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    if (!apiRes?.media_url) {
      return {
        redirect: {
          destination: `/company/process/add?step=1&pid=${apiRes?._id}` +
    ...
    return {
      redirect: {
        destination: `/company/auth/login`,
        permanent: false,
      },
    };
  }
