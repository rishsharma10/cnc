...
const ProcessPage = () => {
  ...
  const { raize, Video, setPic, setClickedTexts } = useContext(GlobalContext);
  const { user_type } = router.query;
  const items2: MenuProps["items"] = [
    {
      key: "1",
      label: (
        <Button type="text" className="text-dark p-0 h-100 text-start">
          Team Admin
        </Button>
      ),
    },
  ...
  const action: MenuProps["items"] = [
    {
      key: "1",
      label: (
        <Link href={`/${user_type}/process/1/view`} className="fw-semibold">
          View
        </Link>
      ),
    },
  ...
  const dataSource = [
    {
      ...
      process_owner: (
        <Flex align="center" gap={8}>
          <Avatar src={profile.src} size={40} />
  ...
  const submit = async () => {
    try {
      ...
      await Video.startScreenRecording();
      await router.push(`/demo/process/add?pid=${apiRes?._id}`);
    } catch (error) {}
  };
  return (
    <React.Fragment>
